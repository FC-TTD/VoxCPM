"""Copied native app.py UI at 8d3237a; only model ownership/callback wiring changes."""
import os
import re
import sys
import logging
...
class VoxCPMDemo:
    def __init__(self, runtime):
        ...
        # The vendored funasr shim is a CPU HTTP client. This argument is native
        # compatibility metadata, not local ASR weight/device placement.
        self.asr_device = "cuda:0"
        self.asr_model: Optional[AutoModel] = None
        self.generate_tts_audio = runtime.task(self.generate_tts_audio)

    def get_or_load_voxcpm(self):
        # No API-error fallback may construct a second, unleased native model.
